...
# Build and Push Script for ComfyUI-QwenVL-Mod (RunPod, CUDA 13.0, LTX 2.5 edition)
# Based on runpod/comfyui:cuda13.0 template

set -e

echo "🐳 Building ComfyUI-QwenVL-Mod Docker image (RunPod, CUDA 13.0, LTX 2.5)..."

# Build variables
IMAGE_NAME="huchukato/comfyui-qwenvl-runpod"
TAG="cu13-ltx25"
DOCKERFILE="Dockerfile.CU13-LTX25"
...
docker buildx build --builder desktop-linux --platform ${PLATFORM} --no-cache --build-arg CACHEBUST=$(date +%s) -f ${DOCKERFILE} -t ${IMAGE_NAME}:${TAG} --load .
