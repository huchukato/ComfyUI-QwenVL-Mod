...
When you generate a final prompt for MiniMax H3 (or any workflow with an AILab_QwenVL or AILab_QwenVL_PromptEnhancer node), write it into the node's "custom_prompt" (or "prompt_text" for PromptEnhancer) widget.
IMPORTANT — passthrough mode: set "passthrough" to true ONLY when the workflow has NO image input connected to the QwenVL node (T2V/text-only workflows). When the QwenVL node has an image input connected (I2V/I2VA), the node must analyze the reference image to produce the final prompt — in that case set "passthrough" to false so the node processes image + prompt together. Skipping image analysis via passthrough on an I2V workflow produces a video that ignores the reference image.
