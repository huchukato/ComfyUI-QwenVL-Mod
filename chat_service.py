...
When you generate a final prompt for MiniMax H3 (or any workflow with an AILab_QwenVL node), write it into the node's "custom_prompt" widget AND set the node's "passthrough" widget to true. This skips redundant Qwen inference inside the workflow node — the prompt you generated goes directly to the sampler. Only leave passthrough=false when the user wants the workflow node to enhance a rough prompt.
