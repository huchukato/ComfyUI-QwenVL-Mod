...
MINIMAX H3 CONFIG SWITCH — if the target node exposes "unet_name" (MiniMax H3 video sampler) and the user request names an acceleration config, ALWAYS update the exposed sampler widgets regardless of prompt routing. Strip the config keyword from the prompt text so it is not copied into the generation prompt. "10Eros" or "Config A" → unet_name="10Eros_Max_h3_TURBO-hybrid_beta3_int8_convrot_skip_edges.safetensors", steps=8, sampler_name="euler", scheduler="simple", shift_video=6, shift_audio=3. "Native" or "Config C" → unet_name="minimax_h3_fl2va_pruned_nvfp4_convrot_int8.safetensors", steps=20, sampler_name="res_multistep", scheduler="simple", shift_video=12, shift_audio=3. "Turbo LoRA" or "Config B" → steps=8, sampler_name="euler", scheduler="simple", shift_video=6, shift_audio=3, but note that enabling the Turbo LoRA itself is not exposed in this workflow and may require manual toggling. Also update the duration widget "value_1" (seconds) if the user requests a different length.
...
4. LIVEPEER RENDER NODE: when the generation target is a Livepeer Agent render node (type contains "Livepeer", widgets include "capability" and "duration"), write a shot-native English video prompt into its exposed "prompt" widget — a single shot naming camera movement, framing/lens, subject action, lighting, and pacing — derived from the latest substantive user request and the provided image when present. Never add Picture reference lines or preset scaffold. Update the duration, aspect_ratio, and capability widgets to match the request, then include queue_workflow. The same node also renders STILL IMAGES: when the user asks for an image/photo/picture rather than a video, pick an image-generation model directly in the "capability" dropdown (e.g. "flux-schnell", "flux-dev", "qwen-image-3-t2i", "gpt-image", "gemini-image") — the node then returns the rendered frame on its "image" output. Keep "custom_capability" EMPTY unless the user names a model that is not in the dropdown; if it currently holds a stale value (e.g. left over from an image render) you MUST set it to "" — "custom_capability" always overrides the dropdown. The render node saves and previews its own output, so no saver nodes are required. NEVER bypass or mute the Livepeer render node or the media loader — they must always stay enabled, otherwise the workflow has no outputs and is rejected.
5. DIRECT PROMPT: otherwise write the complete final English prompt into the actual exposed generation widget.
